...
function patchAwsSdk() {
  ...
  // Patch http.request to fix ES scripted_metric null params (ES 7.x compat)
  var esHost2 = process.env.ES_ENDPOINT ? process.env.ES_ENDPOINT.split(":")[0] : null;
  if (esHost2) {
    var esPort2 = process.env.ES_ENDPOINT ? parseInt(process.env.ES_ENDPOINT.split(":")[1] || "9200", 10) : 9200;
    ...
    http.request = function(options, callback) {
      ...
      var host2 = options.hostname || options.host || "";
      var reqPath = options.path || "/";
      var isEsSearch = (host2 === esHost2 || host2.startsWith(esHost2 + ":")) && reqPath.indexOf("_search") !== -1;

      if (!isEsSearch) return origHttpReq.call(http, options, callback);
      ...
      var origReq = origHttpReq.call(http, options, callback);
      var origWrite = origReq.write.bind(origReq);
      var origEnd = origReq.end.bind(origReq);
      var bodyChunks = [];

      origReq.write = function(chunk, enc, cb) {
        if (chunk) bodyChunks.push(typeof chunk === "string" ? Buffer.from(chunk, enc) : chunk);
        if (cb) cb();
        return true;
      };
      origReq.end = function(chunk, enc, cb) {
        if (chunk) bodyChunks.push(typeof chunk === "string" ? Buffer.from(chunk, enc) : chunk);
        var fullBody = Buffer.concat(bodyChunks).toString("utf-8");
        try {
          if (fullBody.indexOf("scripted_metric") !== -1) {
            var parsed2 = JSON.parse(fullBody);
            _patchNulls(parsed2);
            fullBody = JSON.stringify(parsed2);
          }
        } catch (_) {}
        origReq.setHeader("content-length", Buffer.byteLength(fullBody));
        origWrite(fullBody);
        origEnd(null, null, cb);
      };
      return origReq;
    };
  }
...
function _patchNulls(obj) {
  if (!obj || typeof obj !== "object") return;
  if (Array.isArray(obj)) { obj.forEach(function(item) { _patchNulls(item); }); return; }
  for (var key in obj) {
    if (!obj.hasOwnProperty(key)) continue;
    if (key === "scripted_metric" && obj[key] && obj[key].params) {
      var params = obj[key].params;
      for (var pk in params) {
        if (params.hasOwnProperty(pk) && params[pk] === null) delete params[pk];
      }
    }
    if (typeof obj[key] === "object" && obj[key] !== null) _patchNulls(obj[key]);
  }
}
